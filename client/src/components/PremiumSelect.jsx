...
const PremiumSelect = ({ value = "", onChange, children, className = "", disabled = false, ...rest }) => {
  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 ${open ? "z-[100]" : "z-10"} ${layoutClassName}`}>
      <button type="button" disabled={disabled} aria-haspopup="listbox" aria-expanded={open} aria-label={rest["aria-label"]} onClick={() => setOpen((current) => !current)} onKeyDown={handleKeyDown} className={`premium-select-trigger flex h-full min-h-9 w-full items-center justify-between gap-3 rounded-[0.7rem] border border-white/20 bg-[#081525]/90 px-3 py-2 text-left text-sm text-slate-100 shadow-[inset_0_1px_0_rgba(255,255,255,.08),0_8px_24px_rgba(1,8,20,.3)] backdrop-blur-xl transition hover:border-cyan-300/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/30 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}>
        <span className="min-w-0 truncate">{selected.label}</span><span className={`text-cyan-400 transition-transform ${open ? "rotate-180" : ""}`}>⌄</span>
      </button>
      {open && <div className="absolute left-0 right-0 top-[calc(100%+0.4rem)] z-[999] min-w-[160px] overflow-hidden rounded-xl border border-cyan-400/40 bg-[#081525]/98 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.85),inset_0_1px_0_rgba(255,255,255,.1)] backdrop-blur-2xl" role="listbox" aria-label={rest["aria-label"]}>
        {options.length > 7 && <input autoFocus value={search} onChange={(event) => { setSearch(event.target.value); setActiveIndex(0); }} onKeyDown={handleKeyDown} placeholder="Filter options…" className="mb-1.5 h-8 w-full rounded-lg border border-white/15 bg-slate-950/80 px-2.5 text-xs text-slate-100 outline-none placeholder:text-slate-500 focus:border-cyan-300/60" />}
        <div className="max-h-64 overflow-y-auto overscroll-contain">
          {filtered.map((option, index) => <button key={`${option.value}-${index}`} type="button" role="option" aria-selected={option.value === selected.value} disabled={option.disabled} onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(option)} className={`flex min-h-9 w-full items-center rounded-lg px-2.5 py-2 text-left text-xs transition font-medium ${option.value === selected.value ? "bg-cyan-400/20 text-cyan-200 font-bold" : index === activeIndex ? "bg-white/[0.12] text-white" : "text-slate-200 hover:bg-white/[0.08] hover:text-white"} disabled:opacity-40`}>{option.label}</button>)}
          {!filtered.length && <div className="px-2.5 py-3 text-xs text-slate-400">No options found.</div>}
        </div>
      </div>}
    </div>
  );
