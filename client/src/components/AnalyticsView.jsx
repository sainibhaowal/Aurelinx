...
import { analysisAPI, enterpriseAPI } from "../services/apiClient";
...
const AnalyticsView = () => {
  ...
  useEffect(() => {
    ...
    analysisAPI
      ...
      .catch((error) => {
        console.error("Analytics context load failed", error);
  ...
  useEffect(() => {
    ...
    analysisAPI
      ...
      .catch(() => {
        /* stream remains the live fallback */
      });
    return () => {
      active = false;
    };
  ...
  const riskEmployees = visibleEmployees
    .filter((employee) => employee.is_at_risk)
    .sort(
      (a, b) =>
        Number(a.retention_prob ?? 0.5) - Number(b.retention_prob ?? 0.5),
    );
  const derivedDepartmentRows = departments
    .map((department) => {
      const rows = visibleEmployees.filter(
      ...
      const sentiment = rows.length
        ? rows.reduce((sum, row) => sum + Number(row.sentiment_score || 0), 0) /
          rows.length
        : 0;
      const retention = rows.length
        ? rows.reduce(
            (sum, row) => sum + Number(row.retention_prob ?? 0.5),
            0,
          ) / rows.length
        : 0;
  ...
  const activeDepartmentRows = selectedDepartment
    ? departmentRows.filter((row) => row.department === selectedDepartment)
    : departmentRows;
  const candidateMatchValues = candidateRecords
    .map((candidate) => Number(candidate.match_score))
    .filter((score) => Number.isFinite(score));
  const candidateAverageMatch = candidateMatchValues.length
    ? candidateMatchValues.reduce((sum, score) => sum + score, 0) /
      candidateMatchValues.length
    : serverCandidateAverage;
  const candidateHighMatch = candidateMatchValues.length
    ? candidateMatchValues.filter((score) => score >= 0.7).length
    : serverCandidateHigh;
  const filteredAtRiskCount = isFiltered
    ? serverScopeAtRisk
    : visibleEmployees.filter((employee) => employee.is_at_risk).length;
  const filteredScopeCount = isFiltered
    ? serverScopeTotal
    : visibleEmployees.length;
  const filteredRiskPct = filteredScopeCount
    ? (filteredAtRiskCount / filteredScopeCount) * 100
    : 0;
  const displayedRiskPct = isFiltered ? filteredRiskPct : stats.atRiskPct;
  const displayedRiskLevel =
    displayedRiskPct >= 20 ? "HIGH" : displayedRiskPct >= 10 ? "MEDIUM" : "LOW";
  const displayedTopRisk = [...departmentRows].sort(
    (a, b) =>
      (b.total ? b.risk / b.total : 0) - (a.total ? a.risk / a.total : 0),
  )[0];
  ...
  const exportAnalytics = async (format) => {
    ...
    generateAurelinxReport(
      ...
      `Analytics export: ${visibleEmployees.length} employees; filters: ${departmentFilter || "all departments"}${riskOnly ? ", at-risk only" : ""}.`,
      format,
    );
  };
  ...
  const createRiskIntervention = async (employee) => {
    ...
    try {
        ...
        (int) =>
          ...
          (int.target_employee_id === employee.id ||
            (int.title &&
              int.title
                .toLowerCase()
                .includes(employee.full_name.toLowerCase()))),
      );

      if (isAlreadyCreated) {
        showToast(
          `⚠️ Review request for ${employee.full_name} is ALREADY CREATED in the Reviews Queue!`,
          "warning",
        );
        setActionMessage({
          type: "error",
      ...
      await enterpriseAPI.createIntervention({
        ...
        status: "planned",
        expected_impact:
          "Document a retention conversation and reassess the employee risk signals.",
      });

      showToast(
        `✓ Review request created for ${employee.full_name}! View in Reviews Queue.`,
        "success",
      );
      setActionMessage({
        type: "success",
        text: `Review created for ${employee.full_name}.`,
        employee: employee.full_name,
      });
    } catch (error) {
      ...
      setActionMessage({
        ...
        text:
          error?.status === 403
            ? "Review blocked: administrator approval is required."
  ...
  return (
    <div className="w-full pb-20">
      ...
      {actionMessage && (
        <div
          role="status"
          className={`mb-4 flex flex-wrap items-center gap-3 rounded-lg border px-4 py-3 text-xs ${actionMessage.type === "success" ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : "border-rose-300/20 bg-rose-300/10 text-rose-100"}`}
        >
          ...
          {actionMessage.type === "success" && (
            <button
              ...
              className="rounded-md border border-emerald-200/30 px-2.5 py-1 font-semibold text-emerald-100 hover:bg-emerald-200/10"
            >
        ...
        <div className="flex flex-wrap gap-2">
          {["pdf", "excel", "markdown"].map((format) => (
            <button
              ...
              className="h-10 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold uppercase text-slate-200 disabled:opacity-40"
            >
              {format === "pdf"
                ? "PDF"
                : format === "excel"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        <MetricCard
          ...
          delta={
            isFiltered ? "Filtered employee records" : "Live employee records"
          }
          color="primary"
        />
        <MetricCard
              ...
              >
                <div className="flex justify-between text-sm font-bold">
                  ...
                  <span className="text-white">
                    {dept.total} Members · {dept.risk} risk
                  </span>
                </div>
      ...
      <div className="mt-8 grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6 items-stretch">
        <section className="premium-card p-5 flex flex-col h-[560px]">
          <div className="mb-4 flex items-start justify-between gap-3 shrink-0">
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-500">
                Select a department from the distribution above to focus this
                view.
              </p>
            </div>
            {selectedDepartment && (
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-white/[0.07] pr-2 custom-scrollbar">
            {activeDepartmentRows.map((row) => (
              <div key={row.department} className="py-3 first:pt-0 last:pb-0">
                <div className="flex items-center justify-between gap-3">
                  <span className="font-semibold text-slate-200">
                    {row.department}
                  </span>
                  <span className="text-[10px] text-slate-500">
                    {row.total} employees · {row.risk} at risk
                  </span>
                </div>
                <div className="mt-2 grid grid-cols-3 gap-3 text-[10px] text-slate-500">
                  ...
                  <span>
                    ...
                    <strong className="text-rose-200">
                      {row.total
                        ? ((row.risk / row.total) * 100).toFixed(1)
                        : "0.0"}
                      %
                    </strong>
                  </span>
        ...
        <section className="premium-card p-5 flex flex-col h-[560px]">
          <div className="mb-4 flex items-start justify-between gap-3 shrink-0">
            <div>
              <h3 className="text-sm font-bold text-slate-100">
                Employee risk evidence
              </h3>
              <p className="mt-1 text-[11px] text-slate-500">
                Ordered by lowest retention probability. Review actions require
                administrator approval when high priority.
              </p>
            </div>
            <span className="shrink-0 rounded-full bg-rose-400/10 px-2.5 py-1 text-[10px] font-semibold text-rose-200">
              {riskEmployees.length} matches
            </span>
          </div>
          <div className="flex-1 flex flex-col overflow-y-auto divide-y divide-white/[0.07] pr-2 custom-scrollbar py-1">
            {riskEmployees.map((employee, index) => (
              <div
                key={employee.id}
                className="flex items-center gap-3 py-3 hover:bg-white/[0.03] px-2 rounded-lg transition-colors"
              >
                ...
                <div className="shrink-0 text-right text-[10px]">
                  <div className="font-semibold text-rose-300">
                    Retention{" "}
                    {(Number(employee.retention_prob ?? 0.5) * 100).toFixed(0)}%
                  </div>
                  <button
                    onClick={() => createRiskIntervention(employee)}
                    className="mt-1 font-semibold text-cyan-200 hover:text-cyan-100 transition-colors"
                  >
                    Create review
                  </button>
                </div>
            ...
            {loadingMoreRisk && (
              <p className="py-2 text-center text-[10px] text-cyan-200">
                Loading more evidence…
              </p>
            )}
            {!riskEmployees.length && (
              <p className="py-3 text-xs text-slate-500">
                No at-risk employees match the selected filters.
              </p>
            )}
            <div ref={riskSentinelRef} className="h-1" aria-hidden="true" />
          </div>
        </section>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 xl:grid-cols-2 gap-6">
        <section className="premium-card p-5">
          <div className="mb-4">
            <h3 className="text-sm font-bold text-slate-100">
              Candidate and hiring context
            </h3>
            <p className="mt-1 text-[11px] text-slate-500">
              A separate population from the employee workforce.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-3 border-y border-white/[0.07] py-4 text-xs">
            <span>
              <strong className="block text-lg text-cyan-200">
                {candidateCount ?? "—"}
              </strong>
              <span className="text-slate-500">candidate records</span>
            </span>
            <span>
              <strong className="block text-lg text-cyan-200">
                {candidateAverageMatch == null
                  ? "—"
                  : `${(candidateAverageMatch * 100).toFixed(1)}%`}
              </strong>
              <span className="text-slate-500">average match</span>
            </span>
            <span>
              <strong className="block text-lg text-emerald-200">
                {candidateHighMatch}
              </strong>
              <span className="text-slate-500">high match ≥70%</span>
            </span>
          </div>
          <p className="mt-4 text-[11px] leading-relaxed text-slate-500">
            Candidate match, sentiment, role, and department belong to Talent
            Scout. They are not included in employee morale or risk totals.
            Hiring pipeline stages are not present in the current schema.
          </p>
        </section>
        <section className="premium-card p-5">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-500">
                One bar is one live analytics snapshot captured in this
                workspace. Bar height is the at-risk percentage.
              </p>
            </div>
          ...
          {trend.length ? (
            <div>
              <div className="relative flex h-20 items-end gap-1 border-b border-white/[0.08]">
                {trend.map((point, index) => {
                  const rawRisk = Number(
                    point.atRiskPct ?? point.at_risk_pct ?? 0,
                  );
                  // Normalize risk percentage (convert ratio <= 1.0 to 0..100 percentage)
                  const riskPct =
                    rawRisk <= 1.0 && rawRisk > 0 ? rawRisk * 100 : rawRisk;
                  const heightPct = Math.max(8, Math.min(100, riskPct));
                  return (
                    <div
                      key={`${point.timestamp}-${index}`}
                      className="relative min-w-[4px] flex-1"
                    >
                      <div
                        role="img"
                        tabIndex={0}
                        aria-label={`${new Date(point.timestamp).toLocaleString()}; risk ${riskPct.toFixed(1)} percent`}
                        ...
                        className="h-full cursor-help outline-none"
                      >
                        <div
                          className="absolute bottom-0 left-0 right-0 min-h-[6px] rounded-t bg-gradient-to-t from-rose-500/80 to-amber-300/80 transition-[filter] hover:brightness-125 focus-visible:brightness-125 shadow-[0_0_8px_rgba(251,113,133,0.3)]"
                          style={{ height: `${heightPct}%` }}
                        />
                        {hoveredSnapshot === point.timestamp && (
                          <div className="pointer-events-none absolute bottom-[calc(100%+8px)] left-1/2 z-30 -translate-x-1/2 whitespace-nowrap rounded-lg border border-rose-300/30 bg-[#1b111d]/95 px-3 py-2 text-[10px] leading-relaxed text-slate-200 shadow-xl shadow-rose-950/30 backdrop-blur-md">
            ...
            <div className="flex h-20 items-center justify-center rounded-lg border border-dashed border-white/10 text-xs text-slate-500">
              History will appear after the live stream captures its first
              snapshot.
            </div>
          )}
        </section>
      ...
      <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.02] p-4 text-[10px] leading-relaxed text-slate-500">
        ...
        metrics use the authenticated tenant's filtered employee records.
        Employee values are observed database fields; risk rate, department
        concentration, and risk level are derived calculations. Import
        validation, duplicate checks, and quarantine counts are available in
        Data Ops.
      </div>
    </div>
  );
