  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 ${layoutClassName}`}>
      <button
        ...
        className={`premium-select-trigger flex h-full min-h-9 w-full items-center justify-between gap-3 rounded-[0.7rem] border border-white/20 bg-[#081525]/90 px-3 py-2 text-left text-sm text-slate-100 shadow-[inset_0_1px_0_rgba(255,255,255,.08),0_8px_24px_rgba(1,8,20,.3)] backdrop-blur-xl transition hover:border-cyan-300/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/30 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
      >
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div
          ...
          className="overflow-hidden rounded-xl border border-cyan-400/40 bg-[#081525]/98 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.85),inset_0_1px_0_rgba(255,255,255,.1)] backdrop-blur-2xl"
          role="listbox"
          aria-label={rest["aria-label"]}
        >
          {options.length > 7 && (
