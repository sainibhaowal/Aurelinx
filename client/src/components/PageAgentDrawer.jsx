...
const PageAgentDrawer = ({
  ...
  const markdownComponents = {
    h1: ({ children }) => (
      <h1 className="text-base font-extrabold tracking-tight text-slate-50 mt-0 mb-2">
        {children}
      </h1>
    ),
    h2: ({ children }) => (
      <h2 className="text-sm font-bold tracking-tight text-slate-50 mt-4 mb-2">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-sm font-semibold tracking-tight text-slate-50 mt-3 mb-1.5">
        {children}
      </h3>
    ),
    p: ({ children }) => (
      <p className="my-2 leading-6 text-slate-200">{children}</p>
    ),
    ul: ({ children }) => (
    ...
    blockquote: ({ children }) => (
      <blockquote className="my-3 border-l border-cyan-300/30 pl-3 italic text-slate-300">
        {children}
      </blockquote>
    ),
    code: ({ inline, children }) =>
      inline ? (
        <code className="rounded bg-white/5 px-1.5 py-0.5 font-mono text-[0.92em] text-cyan-100">
          {children}
        </code>
      ) : (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-3 space-y-2">
              {result && (
                <div className="rounded-2xl border border-cyan-300/15 bg-cyan-300/10 p-4 mt-3">
                  <div className="text-[10px] uppercase tracking-[0.2em] text-cyan-200 mb-2">
                    Answer
                  </div>
